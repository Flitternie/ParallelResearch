...
from flash_research import AgenticPlanner, BreadthPlanningDecision
...
class FlashResearchRuntime(RecursiveDeepResearch):
    ...
    async def deep_research(
        ...
        if is_recursive:
            ...
            async with self.semaphore:  # Control concurrency
                try:
                    ...
                    if self.enable_early_termination:
                        ...
                        # Wait for research to complete before harvesting results to avoid partial context/citations
                        await research_task
                    else:
                    ...
                    if depth < self.config.max_depth and current_status not in [TaskState.TERMINATED.value, TaskState.CANCELLED.value]:
                        ...
                        # current_research_context = await self.task_manager.get_all_data()
                        # sub_queries = await self.plan_serp_queries(next_query, max_breadth=new_breadth, current_depth=new_depth, context=current_research_context)
                        ...
                        # Update planning node with generated queries
                        ...
                        # Cancel monitoring task once all children are complete
                        if self.enable_early_termination and 'monitor_task' in locals():
                            monitor_task.cancel()
                            try:
                                await monitor_task
            ...
            # serp_queries = await self.plan_serp_queries(query, max_breadth=breadth, current_depth=depth, context="")
        ...
        if node_results:
            ...
            self.logger.update_node(
                node_id=current_node_id,
                status="completed" if self.logger.get_node_status(current_node_id) == "started" else self.logger.get_node_status(current_node_id),
    ...
    async def _monitor_research_progress(self, researcher: GPTResearcher, query: str, 
        ...
        try:
            ...
            while True:
                await asyncio.sleep(8)  # Check every 5 seconds
